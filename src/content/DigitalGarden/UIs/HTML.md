## Links
- [Don't use `<input type="number"`](https://technology.blog.gov.uk/2020/02/24/why-the-gov-uk-design-system-team-changed-the-input-type-for-numbers/)
- [Layout shifts using web fonts](https://simonhearne.com/2021/layout-shifts-webfonts/)
- [Creating webfont subsets](https://markoskon.com/creating-font-subsets/ )
- [Web Accessibility](https://reactjs.org/docs/accessibility.html)
	- [Color Contrast Checker](https://contrastchecker.com/)
- [Firefox Reader view](https://videoinu.com/blog/firefox-reader-view-heuristics/)
- [Send a request on page exit (beacon, ping)](https://css-tricks.com/send-an-http-request-on-page-exit/)
- [Use `noreferer` and `noopener` when using target blank](https://www.pixelstech.net/article/1537002042-The-danger-of-target=_blank-and-opener )

